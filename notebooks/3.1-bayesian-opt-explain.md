...
Bayesian Optimization builds a probabilistic model of the objective function to intelligently choose the next parameters to evaluate.
Unlike grid or random search, it balances exploration (trying uncertain regions) and exploitation (refining promising areas).
This makes it far more efficient, achieving better results with fewer evaluations.

For further information, we recommend the following resources:

| Reference | What to use it for |
|------------|--------------------|
| [Bayesian Optimization Distill Tutorial](https://distill.pub/2020/bayesian-optimization) (🔥 highly recommended) | A clear and pedagogical introduction to Bayesian Optimization concepts. |
| [\(Snoek et al., 2012\)](https://arxiv.org/abs/1206.2944) | Understanding the theoretical and practical foundations of Bayesian Optimization in ML. |
| [\(Garnett, 2023\)](https://bayesoptbook.com) | A comprehensive theoretical reference on Bayesian Optimization. |
| [\(Watanabe, 2023\)](https://arxiv.org/abs/2304.11127) | Detailed explanation of the Tree-Structured Parzen Estimator (TPE) algorithm. |
| [\(Akiba et al., 2019\)](http://arxiv.org/abs/1907.10902) | Practical implementation details of Bayesian Optimization using Optuna in Python. |

---

## The need for hyperparameter optimization

A **hyperparameter** is a parameter that characterizes a machine learning model. For example, in CatBoost, it could be the number of iterations, while in logistic regression, it could be the regularization factor. Although modern libraries often provide satisfactory default values, exploring other configurations is essential to optimize model performance. Hyperparameter tuning is therefore a central and recurring challenge in machine learning. However, this process is often perceived as "opaque" because it lacks systematic methods and heavily depends on experience and empirical rules [\(Snoek et al., 2012\)](https://arxiv.org/abs/1206.2944).
...
To achieve its goal, Bayesian optimization builds a probabilistic model of $f(\mathbf{x})$, the function to optimize. This function takes its inputs from a set $\mathcal{X} \subset \mathbb{R}^D$. At each iteration, the probabilistic model of $f(\mathbf{x})$ guides the search in $\mathcal{X}$, taking into account both the performance of previous iterations and regions of the search space with more or less uncertainty [\(Snoek et al., 2012\)](https://arxiv.org/abs/1206.2944).

Bayesian optimization relies on two main choices [\(Snoek et al., 2012\)](https://arxiv.org/abs/1206.2944): selecting a **surrogate model** to approximate the function to optimize and choosing an **acquisition function** $\alpha(\mathbf{x})$ to determine the next point to evaluate. **Gaussian processes** are frequently used as surrogate models due to their flexibility and ease of use, with a specified prior over $f(\mathbf{x})$.

The Python library used in this workshop (Optuna) [\(Akiba et al., 2019\)](http://arxiv.org/abs/1907.10902) relies on the *Tree-Structured Parzen Estimator* algorithm as surrogate model. However, this algorithm is not detailed here; for more information, the reader may refer to [\(Watanabe, 2023\)](https://arxiv.org/abs/2304.11127).

In general, one iteration of Bayesian optimization proceeds as follows [\(Agnihotri & Batra, 2020\)](https://distill.pub/2020/bayesian-optimization):
...
Several acquisition functions exist to guide the choice of the next point to evaluate. Here, only the **Probability of Improvement (PI)** and **Expected Improvement (EI)** functions are discussed, as they are frequently used. For more information on other acquisition functions, see chapters 7 and 8 of [\(Garnett, 2023\)](https://bayesoptbook.com).
...
The next point to evaluate, $x_{t+1}$, is the one that maximizes this probability. Mathematically ([Garnett, 2023](https://bayesoptbook.com), [Agnihotri & Batra, 2020](https://distill.pub/2020/bayesian-optimization)):

$$
x_{t+1} = \arg\max_{\mathbf{x} \in \mathcal{X}} \alpha_{PI}(x)
          = \arg\max_{\mathbf{x} \in \mathcal{X}} \Pr[f(x) > \tau]
$$
...
These derivatives can be interpreted as follows [\(Garnett, 2023\)](https://bayesoptbook.com):
...
Thus, the Probability of Improvement tends to be relatively conservative: it favors "safe" solutions, i.e., certain but modest improvements, rather than riskier solutions that could yield larger but less certain gains [\(Garnett, 2023\)](https://bayesoptbook.com).

#### Expected Improvement $\alpha_{EI}(x)$

As explained in the previous section, the Probability of Improvement only considers the likelihood of an improvement, without taking into account its magnitude. The **Expected Improvement (EI)** function addresses this limitation by selecting the next point to explore as the one that maximizes the expected improvement over the threshold $\tau$, defined as the sum of the best observed point $f(x^+)$ and an exploration-exploitation constant $\epsilon$ [\(Agnihotri & Batra, 2020\)](https://distill.pub/2020/bayesian-optimization).

For a Gaussian process used as the surrogate model, the Expected Improvement function is expressed using the cumulative distribution function $\Phi$ and the probability density function $\phi$ of the standard normal distribution [\(Garnett, 2023\)](https://bayesoptbook.com):
...
The value of $\alpha_{EI}(x)$ is high in two main situations (for $\sigma > 0$) [\(Agnihotri & Batra, 2020\)](https://distill.pub/2020/bayesian-optimization):
...
These explanations conclude this short introduction to Bayesian Optimization. The key takeaway is that this method balances exploration and exploitation through two main components: a surrogate model and an acquisition function.

## References

1. Agnihotri, A. & Batra, N. (2020), ‘Exploring bayesian optimization’, Distill .
https://distill.pub/2020/bayesian-optimization.

2. Akiba, T., Sano, S., Yanase, T., Ohta, T. & Koyama, M. (2019), ‘Optuna: A next-generation
hyperparameter optimization framework’, CoRR abs/1907.10902.
...
5. Watanabe, S. (2023), ‘Tree-structured parzen estimator: Understanding its algorithm components and
