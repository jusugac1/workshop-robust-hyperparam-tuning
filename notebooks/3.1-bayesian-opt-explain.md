...
3. Use the acquisition function $\alpha(\mathbf{x})$ to select the next point $$$\mathbf{x}_{t+1}$$$ to evaluate by solving: $\mathbf{x}_{t+1} = \arg\max_{\mathbf{x} \in \mathcal{X}} \alpha(\mathbf{x})$, where $\mathcal{X}$ is the hyperparameter search space.
